...
nameToInt = sum . map alphToInt
  where alphToInt c = Char.ord c - Char.ord 'A' + 1

totalScore :: [String] -> Int
totalScore = sum . zipWith (\i s -> i * nameToInt s) [1..]
...
prob023 :: Int
prob023 = IntSet.foldr (+) 0 diff
  where 
    diff = IntSet.difference (IntSet.fromList data023) (IntSet.fromList sumOfTwoAbundants)
    sumOfTwoAbundants = [k | i <- abundantList, 
                             j <- abundantList, 
                             i <= j, 
                             let k = i + j, 
                             k <= 28123]
...
--
-- >>> take 6 $ Utils.fibonacciSequence
-- [1,1,2,3,5,8]
...
select n  = return $ [prob021,       0, prob023, prob024, prob025, 
                      prob026, prob027, prob028, prob029, prob030] !! (n - 21)
